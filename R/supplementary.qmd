...
$$
with the site-level random intercepts $\alpha_{i}$ for each response linked together through a common variance-covariance matrix $\boldsymbol{\Omega}_{i}$:
$$
...
Finally, we can model the proportion of **F**ood consumed $F_{i,j,k}$ (site $i$, snail $j$, trial $k$) and movement **A**ctivity $A_{i,j,k}$ as follows:
...
\end{bmatrix}.
$$
We set a $\mathrm{Normal}(0,1.5)$ prior for $\beta_{0[F]}$ (interpretable as the logit of a proportion, see **Model 2**) and $\mathrm{Normal}(0,1)$ priors for all other $\beta$s. As with previous models, we used  $\mathrm{Half-Normal}(0,1)$ priors for random effect SDs $\sigma$ and an $\mathrm{LKJ}(2)$ prior for the correlation matrices $\boldsymbol{R}$. We set $\mathrm{Exponential}(1)$ priors for both $1/\phi$ and $\theta$; note that for the Beta model we put the prior on the inverse of the precision (i.e. the dispersion), rather than the precision itself, to avoid setting too much much prior mass on high dispersion (see e.g. <https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations#story-when-the-generic-prior-fails-the-case-of-the-negative-binomial>, \[accessed 2024-10-16\]).


# S4 - Overview of parasite prevalences

**Table S4-1.** Observed prevalences of each parasite type in sampled _Cornu aspersum_ snails, both per site and overall. Sites are ordered by increasing distance from the city centroid ("Distance (m)" column).
...
# S5 - Investigating the site-level correlation between reflectance and _N. bakeri_ prevalence
...
Running this model gives an estimate for the among-sites correlation of `r print_summarised_cor`, which is not different from zero (**Fig. S5-1**). While this does not necessarily mean that the link we uncovered in the main text is primarily due to _within-population_ correlations (especially given the low number of sites with non-zero prevalence, **Supplementary Material S4**), it does suggest this may be the case.

```{r}
#| echo: false
#| message: false
#| output: false
### a draft version of a code for a figure, ignored for now
# mod_corr$data |>
#  select(SiteID) |>
#  distinct() |>
#  mutate(sigma_reflect_scaled=1) |>
#  add_epred_draws(mod_corr) |>
#  group_by(.category,SiteID) |>
#  mean_qi() |>
#  select(SiteID,.category,starts_with(".epred")) |>
#  pivot_wider(names_from=.category,values_from = c(".epred",".epred.lower",".epred.upper")) |>
#  ggplot()+
#  geom_point(
#    data=mod_corr$data |>
#      group_by(SiteID) |>
#      summarise(mean_reflect=mean(reflect_scaled),mean_nemhelix=mean(nematodes_genitalia)),
#    aes(mean_reflect,mean_nemhelix),
#    col="grey")+
#  geom_segment(
#    aes(
#      x=.epred_reflectscaled,
#      xend=.epred_reflectscaled,
#      y=.epred.lower_nematodesgenitalia,
#      yend=.epred.upper_nematodesgenitalia
#      ))+
#  geom_segment(
#    aes(
#      x=.epred.lower_reflectscaled,
#      xend=.epred.upper_reflectscaled,
#      y=.epred_nematodesgenitalia,
#      yend=.epred_nematodesgenitalia)
#    )+
#  geom_point(
#    aes(
#      .epred_reflectscaled,.epred_nematodesgenitalia),
#    pch=21,fill="white",size=4)+
#  scale_x_continuous("Reflectance (scaled, for now)")+
#  scale_y_continuous("Nemhelix prevalence")+
#  theme_bw()
```
...
ggplot(posterior_cor) +
  stat_halfeye(aes(x = cor), .width = c(0, 0.95)) +
...
**Figure S5-1.** <!--Left: site-level _Nemhelix bakeri_ and average shell reflectance. Grey dots are observed means (ignoring measurement uncertainty for prevalence), while white dots and segments are model predictions and their 95% credible intervals. Right: -->Posterior distribution of the site-level correlation between _N. bakeri_ prevalence and shell reflectance.
